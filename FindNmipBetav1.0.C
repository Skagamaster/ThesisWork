...
void FindNmipBeta(int day=113){
  ...
  TString pathSave = Form("/mnt/d/14GeV/Day%d/Runs/",day);
  //
  ...
  Int_t FirstIterator = 20; // How much to step by when finding the dip after dark current
  ...
  for (int ew=1; ew<2; ew++){
    for (int PP=5; PP<7; PP++){
      int iPad=0;
      theCanvas->cd(++iPad);

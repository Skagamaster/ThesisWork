...
//_________________
void FXTRunAnalysis(const Char_t *inFile = 
"/mnt/d/FXT/27GeV/Picos/st_physics_19138032_raw_0000003.picoDst.picoDst.root") {
  ...
  /* Turned off for now; no nMips yet.
  ...
  // File for normalising the nMips
    ...
    std::ifstream input(Form("/mnt/d/FXT/NmipConstantsDay%d.txt",Tacos));
  ...
  for (int ew=0; ew<2; ew++){
    ...
    mAdcDists[ew][pp-1][tt-1]  = new TH1D(Form("AdcEW%dPP%dTT%d",ew,pp,tt),
      Form("AdcEW%dPP%dTT%d",ew,pp,tt),512,0,4096);
    //mNmipDists[ew][pp-1][tt-1] = new TH1D(Form("nMipEW%dPP%dTT%d",ew,pp,tt),
    //  Form("nMipEW%dPP%dTT%d",ew,pp,tt),512,0,40);
      }
    }
  }
  /* If we want RefMult or pT cuts.
  ...
  */
  ...
  if (Tacos < 100)
  {
    TString pathSave = Form("/mnt/d/FXT/27GeV/Day%d/",Tacos);

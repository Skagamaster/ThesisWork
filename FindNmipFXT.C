...
void FindNmipFXT(int day=138){
  ...
  std::ofstream NmipFile(Form(
    "/mnt/d/FXT/3_85GeV/NmipConstantsDay%d.txt",day),ofstream::out);
  ...
  theCanvas->SaveAs(
        Form("/mnt/d/FXT/3_85GeV/ADCspectraDay%d.pdf[",day));
  TFile* in = new TFile(Form(
            "/mnt/d/FXT/3_85GeV/Day%d.root",day),"READ");
        ...
        for (int TT=1; TT<32; TT++){
          TPad* thePad = (TPad*)theCanvas->cd(++iPad);
            ...
            else
            {
        	   FitRangeLow=80;
        	   FitRangeHigh=1500;
        	   SingleMipPeakStartingValue=110;
        	   MaxPlot=400;
            }
        	}

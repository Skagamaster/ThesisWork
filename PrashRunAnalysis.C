...
//_________________
void PrashRunAnalysis(const Char_t *inFile = 
"/mnt/d/14GeV/110/st_physics_20110001_raw_1000002.picoDst.root") {


  // Array for ADC values (currently not used)
  //double data[4464];
  //double ADCs[2][12][31];
...
/// File for normalising the nMips (not used)
/*std::ifstream input(Form("/mnt/d/14Gev/NmipConstantsDay%d.txt",Tacos));
  ...
  /// 1D histograms for nMip distributions (not used)
  ...
  for (int ew=0; ew<2; ew++){
    ...
    //  Form("nMipEW%dPP%dTT%d",ew,pp,tt),1024,0,40);
      }
    }
  }
  /*
  ...
  */
  ...
  if (Tacos < 100)
  {
    //TString pathSave = Form("/mnt/d/14GeV/%s/",Queso.c_str());
    TFile *MyFile = TFile::Open("test","RECREATE");
    for (int ew=0; ew<2; ew++){
    for (int pp=1; pp<13; pp++){
      for (int tt=1; tt<32; tt++){

...
void DayFitsHistos(const Char_t *inFile = "/mnt/d/14GeV/NmipTotal.txt")
{
...
std::ofstream NmipFile(Form("/mnt/d/14GeV/ChiFit/Nmip_Day_%d.txt",dayStart),ofstream::out);
//double data[4464];
  ...
  for (int j = 0; j < 12; ++j)
  {
    ...
    hADCDelta[l] = new TH2D(Form("hADCDelta%d",l),
      Form("1st MIP MPV Delta for EW%d, PP%d;Day;TT",i,j+1),
        daysUsed,dayStart,dayEnd+1,31,1,32);
  }
}
...
c2->SaveAs(Form("/mnt/d/14GeV/ChiFit/ADCDeltaTest_%d_%d.pdf[",dayStart,dayEnd));
for (int i = 0; i < 2; ++i)
{
  for (int j = 0; j < 12; ++j)
  {

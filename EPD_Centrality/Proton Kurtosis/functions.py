...
def savGol(nSigmaProton):
    binVal = 1000
    counts, bins = np.histogram(
        nSigmaProton, bins=binVal, range=(-10000, 10000))
    x = bins[:-1]
    sg = sgf(counts, 201, 4)
    maxes = np.asarray(
        x[np.hstack(np.asarray(arex(sg, np.greater, order=10)))])
    # for l in maxes:
    #    plt.axvline(l, color='red')
    # plt.hist(nSigmaProton, bins=binVal, range=(-10000, 10000), histtype='step',
    #         label='raw')
    # plt.plot(x, sg, color='purple', label="Smoothed")
    # plt.legend()
    # plt.show()
    # Now take the max closest to 0.
    mini = np.where(np.absolute(maxes) == np.min(np.absolute(maxes)))
    return maxes[mini][0]

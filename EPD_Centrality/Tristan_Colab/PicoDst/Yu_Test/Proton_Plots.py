...
prot = protons['protons'].to_numpy()
aprot = protons['antiprotons'].to_numpy()
...
# This is to compare with Yu's analysis, where he has a cumulant for every integer
# in RefMult3.
full_pro = [[], [], [], []]
full_anti = [[], [], [], []]
full_net = [[], [], [], []]
x = np.sort(np.unique(refmult3))
for i in x:
    index = refmult3 == i
    full_pro[0].append(np.mean(prot[index]))
    full_pro[1].append(np.var(prot[index]))
    full_pro[2].append(skew(np.hstack(prot[index])) * np.power(np.sqrt(np.var(prot[index])), 3))
    full_pro[3].append(kurtosis(np.hstack(prot[index])) * np.power(np.var(prot[index]), 2))
    full_anti[0].append(np.mean(aprot[index]))
    full_anti[1].append(np.var(aprot[index]))
    full_anti[2].append(skew(np.hstack(aprot[index])) * np.power(np.sqrt(np.var(aprot[index])), 3))
    full_anti[3].append(kurtosis(np.hstack(aprot[index])) * np.power(np.var(aprot[index]), 2))
    full_net[0].append(np.mean(net_protons[index]))
    full_net[1].append(np.var(net_protons[index]))
    full_net[2].append(skew(np.hstack(net_protons[index])) * np.power(np.sqrt(np.var(net_protons[index])), 3))
    full_net[3].append(kurtosis(np.hstack(net_protons[index])) * np.power(np.var(net_protons[index]), 2))
full_pro = np.array(full_pro)
full_anti = np.array(full_anti)
full_net = np.array(full_net)
np.save("full_pro.npy", full_pro)
np.save("full_anti.npy", full_anti)
np.save("full_net.npy", full_net)
np.save("x.npy", x)
plt.plot(x, full_net[0])
plt.show()

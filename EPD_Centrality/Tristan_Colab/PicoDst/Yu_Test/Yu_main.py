...
dataDirect = r"C:\Users\dansk\Documents\Thesis\Protons\WIP\FromYu"
dataDirect = r'E:\2019Picos\14p5GeV\Runs'
saveDirect = r"C:\Users\dansk\Documents\Thesis\Protons\WIP\FromYu\Test_Results"
...
# files = ["20118054.picoDst.root"]  #, "20127023.picoDst.root"]
files = os.listdir()
...
for file in files:
    ...
    try:
        ...
        if pico.run_id not in run_list:
            print("File", pico.run_id, "not in Yu's list.")
            continue
        runs = np.hstack((runs, pico.run_id)).astype('int')
        runs = runs[runs != np.array(None)]  # Eliminate runs with errors.
        pico.calibrate_nsigmaproton_yu()
        ...
        file = r"C:\Users\dansk\Documents\Thesis\Protons\WIP\FromYu\Test_Results\\"
